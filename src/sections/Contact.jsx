...
const Contact = ({ lang }) => {
  const [info, setInfo] = useState({ phone: '01732-483149', address: 'Moulvibazar, Sylhet', whatsapp: '', mapLink: '' });
      ...
      <div className="bg-[#111111] rounded-[50px] p-8 lg:p-16 text-white grid md:grid-cols-2 gap-12 items-center shadow-[0_20px_50px_rgba(0,0,0,0.3)] border border-[#D4AF37]/20 relative overflow-hidden">
        ...
        <motion.div initial={{ opacity: 0, x: -30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} className="relative z-10">
          ...
          <div className="space-y-8">
            <a href={`tel:${info.phone}`} className="flex items-center gap-6 group">
            ...
            {info.whatsapp && (
              <a href={`https://wa.me/${info.whatsapp}`} target="_blank" rel="noreferrer" className="flex items-center gap-6 group">
                <div className="w-14 h-14 bg-[#D4AF37]/10 border border-[#D4AF37]/20 rounded-2xl flex items-center justify-center text-[#D4AF37] group-hover:bg-[#25D366] group-hover:text-white transition-all duration-300">
                  <Lucide.MessageSquare size={24} />
                </div>
                <div>
                  <p className="text-xs uppercase font-bold text-[#D4AF37] tracking-widest">WhatsApp</p>
        ...
        <motion.div initial={{ opacity: 0, scale: 0.95 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} className="bg-white rounded-[40px] p-8 lg:p-10 text-gray-900 shadow-2xl relative z-10 border-t-8 border-[#D4AF37]">
          <form className="space-y-5" onSubmit={handleSubmit}>
            ...
            <div>
              ...
              <textarea name="message" required value={formData.message} onChange={handleChange}
                ...
                rows="4" className="w-full p-4 bg-gray-50 rounded-2xl border-2 border-transparent focus:border-[#D4AF37] focus:bg-white focus:outline-none transition-all resize-none"></textarea>
            </div>
            <button type="submit" disabled={loading}
              className={`w-full bg-[#111111] text-[#D4AF37] py-5 rounded-2xl font-black uppercase tracking-[3px] hover:bg-[#D4AF37] hover:text-black shadow-lg transition-all duration-500 transform active:scale-95 flex items-center justify-center gap-2 ${loading ? 'opacity-50' : ''}`}>
